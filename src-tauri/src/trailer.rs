...
pub async fn fetch_trailer(
    ...
    // ANDROID FORK: `transcode` (desktop ffmpeg sidecar lookup) isn't compiled
    // on Android, and the yt-dlp/ffmpeg sidecar flow this function relies on
    // needs a mobile-appropriate replacement anyway (see README-ANDROID.md).
    // Treat ffmpeg as unavailable there so this still compiles.
