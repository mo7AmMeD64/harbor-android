...
### ✅ إصلاح ٦: كل طلبات الشبكة تفشل — `hickory-dns` ما يشتغل على أندرويد
**بعد ما حلّينا مشكلة الإقلاع، ظهرت مشكلة شبكة عامة:** تثبيت أي إضافة
(Torrentio، Cinemeta عبر رابط يدوي...) يفشل بـ"Couldn't read that addon
URL" / "Install failed"، وحتى تصفّح الكتالوج نفسه يطلع صفحة Oops كاملة
برسالة `error sending request for url (https://v3-cinemeta.strem.io/...)`،
وحتى مزامنة إضافات حساب Stremio تطلع "0 addons synced" رغم وجود إضافات
فعلية بالحساب.

**السبب:** كل هذي الأعراض المختلفة سببها **نفس المشكلة الجذرية**:
`http_fetch.rs` و`stream_proxy.rs` يبنون عميل `reqwest` بالإعداد
`.hickory_dns(true)` — يعني يفرض استخدام محلل DNS مكتوب بـ Rust بالكامل
(`hickory-dns`) بدل محلل النظام. هذا المحلل يعتمد على قراءة ملف
`/etc/resolv.conf` لمعرفة سيرفرات DNS — وهذا الملف **غير موجود بنفس الشكل
على أندرويد** (أندرويد يدير DNS بطريقة مختلفة تماماً عبر خصائص النظام). فـ
النتيجة: كل طلب HTTP/HTTPS يفشل فوراً بخطأ عام "error sending request"،
بغض النظر عن الرابط أو الإضافة.

**الحل:** غيّرت السطرين لـ `.hickory_dns(cfg!(not(target_os =
"android")))` — يعني يبقى `true` بسطح المكتب (زي ما كان بالضبط)، ويصير
`false` بس على أندرويد، فيرجع `reqwest` تلقائياً لمحلل DNS الافتراضي
(يعتمد على `getaddrinfo` عبر النظام — نفس الطريقة اللي كل تطبيقات أندرويد
الثانية تستخدمها، مضمونة تشتغل). فحصت باقي عملاء `reqwest` بالمشروع (١٧
موقع) — هذولا الاثنين بس كانوا يفعّلون hickory-dns صراحة؛ الباقي أصلاً
يستخدم المحلل الافتراضي الآمن من البداية.

**هذا الإصلاح متوقع يحل الأعراض الثلاثة كلها دفعة وحدة** (تثبيت الإضافات،
تصفح الكتالوج، مزامنة حساب Stremio) لأنها كلها تمر عبر نفس دالة
`http_client()` بملف `http_fetch.rs`.

## تحسين ١: تبسيط الواجهة للهاتف (لا يعالج خطأ CI، تحسين UX)

**السبب:** التطبيق مصمم بالكامل لسطح مكتب عريض — شريط جانبي طويل بـ١٤
أيقونة، وأزرار تصغير/تكبير/إغلاق نافذة (`chrome/topbar.tsx`) لا معنى لها
بجوال. فحصت `chrome/sidebar.tsx` ولا يوجد فيه أي منطق تكيّف مع الموبايل
إطلاقاً.

**الحل:**
- ملف جديد `chrome/mobile-tab-bar.tsx`: شريط تنقل سفلي بسيط (٤ أزرار
  رئيسية: الرئيسية/اكتشاف/التنزيلات/الإعدادات + زر "المزيد" يفتح قائمة
  بباقي التبويبات: أفلام، مسلسلات، أطفال، أنمي، بث مباشر، قوائم تشغيل،
  التقويم، المكتبة، الإضافات). يعيد استخدام نفس منطق `NAV_ITEMS`
  والتخصيص والقفل الأبوي الموجود أصلاً بـ `sidebar.tsx`، فيبقى سلوكه
  متطابق مع سطح المكتب.
- `App.tsx`: كل عائلة الشريط الجانبي (Sidebar, Dracula/Nord/Forest/Stremio,
  TopDock, CinematicOverlay, RoyalTopbar, SideRail, MinUIDock, FloatingBack)
  + `WindowControls`/`WindowResizeEdges` صارت تظهر بس على سطح المكتب
  (`isDesktopTauri()`)، وتستبدل بـ `MobileTabBar` على أندرويد.
- `chrome/topbar.tsx`: هذا الملف هو المصدر الحقيقي لأزرار تصغير/تكبير/
  إغلاق النافذة اللي كانت تظهر فوق (مو نظام التشغيل كما ظننت بالبداية) —
  صارت تختفي على أندرويد، وأزلت الـ padding الزايد اللي كان يحجز مساحة
  لعرض شريط جانبي غير موجود.
- أضفت مفتاح ترجمة `chrome.more` بالثلاث لغات (عربي/إنجليزي/برتغالي).

**لسه ناقص (تحسينات لاحقة، مو أخطاء):** صفحة الإعدادات (`views/settings/`)
نفسها فيها تخطيط "قائمة + تفاصيل" بعمودين ثابتين (٥٧٥٠ سطر بملف
`settings/nav.tsx` وحده) — هذا مشروع تصميم منفصل وأكبر بكثير، ما لمسته
بهذي الجولة. أزرار تصغير/تكبير/إغلاق داخل مشغّل الفيديو نفسه
(`components/player/transport/window-control-buttons.tsx`) بتتحل مع
مهمة المشغّل القادمة.
